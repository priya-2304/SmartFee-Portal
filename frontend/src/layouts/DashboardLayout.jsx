...
import {
  ...
  FiBarChart2, FiAlertCircle,FiAward,
} from 'react-icons/fi';
...
const staffLinks = [
  ...
  { to: '/admin/fee-structure', label: 'Fee Structure', icon: FiFileText },
  { to: '/admin/transactions', label: 'Transactions', icon: FiUsers },
  { to: '/profile', label: 'Profile', icon: FiUser },
  { to: '/settings', label: 'Settings', icon: FiSettings },
];
...
const DashboardLayout = () => {
 ...
 const links = user?.role === 'admin' ? staffLinks : studentLinks;
      ...
      <aside
        ...
        <div className="p-3 border-t border-white/20 dark:border-gray-700/50 space-y-1">
          <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-gray-50 dark:bg-gray-800/60">
            {user?.profilePicUrl ? (
              <img src={user.profilePicUrl?.startsWith('http')? user.profilePicUrl:`http://localhost:5000${user.profilePicUrl}?t=${Date.now()}`} alt="" className="w-8 h-8 rounded-full object-cover" />):(
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:max-h-screen lg:overflow-y-auto">
        <header className="glass sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b border-white/30 dark:border-gray-700/50">
          ...
          <div className="flex items-center gap-2">
            ...
            {user?.profilePicUrl ? (
              <img
               src={user.profilePicUrl?.startsWith('http') ? user.profilePicUrl : `http://localhost:5000${user.profilePicUrl}?t=${Date.now()}`}
                alt=""
